...
def _processing_data(INTVL_, data_):
    ...
    values_    = INTVL_['MW'][index_].to_numpy()
...
# Select ranked node by distance
...
file_name = path_to_lmp + region + r'-' + market + r'-' + node + r'.pkl'
print(file_name)

# Define url request for real-time 5-minutes LMPs
query  = 'PRC_LMP'
query  = r'&queryname={}&version=12'.format(query)
...
for i in range(N_requests):
    ...
    try:
        ...
        DAM_ = _download_csv(url, zip_file_name = 'temp_{}.zip'.format(i_node))
        # Processing requested data
        data_ = _processing_data(DAM_, data_)
