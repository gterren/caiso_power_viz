import pickle, rasterio, s3fs, pytz, zarr, requests
import tempfile, zipfile, glob, os, sys, xarray, bz2
...
# Define output paths
path_to_output = r"/Volumes/Extreme SSD/caiso_power/output/"
path_to_oac = path_to_output + r"actuals/"
path_to_ofc = path_to_output + r"forecasts/"
path_to_oax = path_to_output + r"auxiliary/"
...
def _extrapolate_wind(u_10_, v_10_, u_80_, v_80_):
    ...
    print(m_10_.shape, m_80_.shape)
    # Compute power law exponent
    alpha_ = (np.log(m_10_) - np.log(m_80_))/(np.log(80.) - np.log(10.))
    print(alpha_.min(), alpha_.max())
    # Compute wind speed applying power law
    m_60_  = __power_law(m_80_, 80., 60., alpha_)
    m_100_ = __power_law(m_80_, 80., 100., alpha_)
    m_120_ = __power_law(m_80_, 80., 120., alpha_)
...
# Find coordinates within California region
    ...
    if not os.path.isfile(path + pickle_file_name):
        ...
        for cycle in cycles_:
            ...
            for forecast in forecasts_:
                ...
                # Get wind speeds in matrix form
                ...
                # Get comfort metrics in matrix form
